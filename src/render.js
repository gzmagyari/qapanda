...
const { summarizeClaudeEvent, summarizeCodexEvent, formatToolCall } = require('./events');
...
class Renderer {
  ...
  line(label, text, labelColor) {
    ...
    this._logChat({ type: 'line', label, text });
  }
  ...
  mdLine(label, text, labelColor) {
    this.flushStream();
    this._ensureHeader(label, labelColor);
    this._logChat({ type: 'mdLine', label, text });
  ...
  launchClaude(prompt, sameSession, agentId, agentCli, agentName, overrideLabel) {
    ...
    this.line(label, `${prefix}"${prompt}"`, color.controller);
  }
  ...
  claudeEvent(raw) {
    ...
    if (summary.kind === 'block-stop') {
      ...
      if (tc) {
        ...
        this._contentLine(`${color.dim}${desc}${color.reset}`);
        this._logChat({ type: 'toolCall', label: this.workerLabel, text: desc });
        this._toolCalls.delete(summary.index);
      }
      return;
    }
