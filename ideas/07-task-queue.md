...
- The queue is persisted to `.qpanda/queue.json` so it survives VS Code reloads.
...
- Add `src/queue.js` with `add(stateDir, message)`, `next(stateDir)`, `list(stateDir)`, `remove(stateDir, index)`. The queue is stored as `.qpanda/queue.json`.
