...
const { createTranscriptRecord } = require('../../src/transcript');
...
describe('Transcript writing', () => {
  ...
  it('transcript file is created during run', async () => {
    ...
    const entry1 = createTranscriptRecord({
      kind: 'user_message',
    ...
    const entry2 = createTranscriptRecord({
    ...
    assert.equal(lines[0].kind, 'user_message');
    assert.equal(lines[0].text, 'Hello');
    assert.equal(lines[1].kind, 'assistant_message');
  ...
  it('transcript entries have correct format for all roles', async () => {
    ...
    assert.equal(lines[0].kind, 'user_message');
    assert.equal(lines[1].kind, 'controller_message');
    assert.equal(lines[1].controllerCli, 'codex');
    assert.equal(lines[2].kind, 'assistant_message');
...
describe('Transcript writing with real worker', { timeout: 60000 }, () => {
  it('runDirectWorkerTurn writes transcript entries', async (t) => {
    ...
    const workerEntry = lines.find(l => l.kind === 'assistant_message');
