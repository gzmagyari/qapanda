...
function handleToolCall(name, args) {
  ...
  switch (name) {
    ...
    case 'update_step_result': { const t=data.tests.find(t=>t.id===args.test_id); if(!t)return JSON.stringify({error:'Not found'}); const r=t.runs.find(r=>r.id===args.run_id); if(!r)return JSON.stringify({error:'Run not found'}); const sr=r.stepResults.find(sr=>sr.stepId===args.step_id); if(!sr)return JSON.stringify({error:'Step not in run'}); if(VALID_STEP_STATUSES.includes(args.status))sr.status=args.status; if(args.actualResult!==undefined)sr.actualResult=args.actualResult; const step=t.steps.find(s=>s.id===args.step_id); if(step){step.status=sr.status; if(args.actualResult!==undefined)step.actualResult=args.actualResult;} t.updated_at=nowIso(); saveData(data); return JSON.stringify({step_id:args.step_id,status:sr.status}); }
    case 'complete_test_run': { const t=data.tests.find(t=>t.id===args.test_id); if(!t)return JSON.stringify({error:'Not found'}); const r=t.runs.find(r=>r.id===args.run_id); if(!r)return JSON.stringify({error:'Run not found'}); if(args.notes)r.notes=args.notes; const sts=r.stepResults.map(sr=>sr.status); if(sts.every(s=>s==='pass'||s==='skip'))r.status='passing'; else if(sts.every(s=>s==='fail'))r.status='failing'; else if(sts.some(s=>s==='fail'))r.status='partial'; else r.status='untested'; t.status=computeOverallStatus(t.steps); t.updated_at=nowIso(); saveData(data); return JSON.stringify({test_id:t.id,run_id:r.id,status:r.status,test_status:t.status},null,2); }
