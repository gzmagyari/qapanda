...
class ClaudeSession {
  ...
  constructor(opts = {}) {
    ...
    this.startupTimeout = opts.startupTimeout || 60000;
  ...
  async send(prompt, callbacks = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const idlePoll = setInterval(() => {
        ...
        // Must have our prompt echoed (prompt char may be ❯ or >)
        const promptIdx = lines.findIndex(l => (l.startsWith('❯') || l.startsWith('>')) && l.includes(shortPrompt));
        if (promptIdx === -1) return;

        // Must have a response started (● or other content after prompt)
        const responseIdx = lines.findIndex((l, i) => i > promptIdx && (l.startsWith('●') || l.startsWith('>')));
  ...
  _emitClassified(classified, callbacks, toolIndex) {
    ...
    if (classified.type === 'text') {
      ...
      const toolName = classified.mcpName || (parsed ? parsed.name : 'unknown');
      callbacks.onEvent({
        source: 'worker',
        kind: 'tool-start',
        toolName,
  ...
  _isIdle(lines) {
    ...
    const prompt = between[0].trimEnd();
    // Claude TUI may render the prompt as ❯ or > depending on terminal capabilities
  ...
  _waitForIdle(timeout) {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        const lines = this._snapshotBuffer();
        const last20 = lines.slice(-20).map((l, i) => `  [${i}] ${JSON.stringify(l)}`).join('\n');
        reject(new Error(`ClaudeSession: timed out waiting for idle state (${timeout}ms). Buffer last 20 lines:\n${last20}`));
      }, timeout);

      const poll = setInterval(() => {
        const lines = this._snapshotBuffer();

        // Auto-confirm workspace trust prompt if it appears
        if (lines.some(l => l.includes('Yes, I trust this folder'))) {
          this._pty.write('\r');
          return; // wait for next poll to check idle
        }
  ...
  _getResponseText(lines, shortPrompt) {
    ...
    // Prompt char may be ❯ or > depending on terminal capabilities
    let promptIdx = -1;
    for (let i = lines.length - 1; i >= 0; i--) {
      if ((lines[i].startsWith('❯') || lines[i].startsWith('>')) && lines[i].includes(shortPrompt)) {
        promptIdx = i;
        break;
      }
    }
