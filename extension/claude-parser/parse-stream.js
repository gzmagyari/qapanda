...
function classifyLine(line) {
  ...
  // User prompt line — skip (prompt char may be ❯ or >)
  if (t.startsWith('❯')) return null;
  if (t.startsWith('>') && (t === '>' || t.startsWith('> '))) return null;
  ...
  if (t.startsWith('●')) {
    ...
    // Tool calls: "Bash(...)", "Write(...)", "mcp__server__tool(...)", etc.
    if (/^(Bash|Write|Read|Edit|Update|Glob|Grep|WebFetch|WebSearch|TodoWrite|mcp__\w+)\s*[\(\[]/.test(inner)) {
      return { type: 'tool', text: inner };
    }
    // MCP tool calls in TUI format: "server-name - method_name (MCP)" or "server - method (MCP)(args)"
    const mcpMatch = inner.match(/^([\w-]+)\s+-\s+(\w+)\s+\(MCP\)/);
    if (mcpMatch) {
      const mcpName = `mcp__${mcpMatch[1]}__${mcpMatch[2]}`;
      return { type: 'tool', text: inner, mcpName };
    }
    return { type: 'text', text: inner };
  }

  // MCP tool calls without ● prefix (continuation lines showing tool name)
  if (/^[\w-]+\s+-\s+\w+\s+\(MCP\)/.test(t)) {
    const mcpMatch = t.match(/^([\w-]+)\s+-\s+(\w+)\s+\(MCP\)/);
    if (mcpMatch) {
      const mcpName = `mcp__${mcpMatch[1]}__${mcpMatch[2]}`;
      return { type: 'tool', text: t, mcpName };
    }
  }
...
function parseToolCall(text) {
  ...
  // MCP TUI format: "server-name - method (MCP)" or "server-name - method (MCP)(args)"
  const mcpMatch = text.match(/^([\w-]+)\s+-\s+(\w+)\s+\(MCP\)(?:\(([^]*)\))?/);
  if (mcpMatch) return { name: `mcp__${mcpMatch[1]}__${mcpMatch[2]}`, argsText: mcpMatch[3] || '' };
  return null;
}
